...
$(document).ready(function () {
  ...
  $(document).on("click",".logo-360",function (event) {
  ...
  $(document).on("click",".logo-360.close-view, .close-360, .close-view",function (e) {
    e.stopPropagation();
    $('body, html').css({ 'heigh': 'auto', 'overflow': 'visible' });
  
    $('#page').css({ 'position': 'static' });
    console.log(posEdificio2)
    $('body, html').scrollTop(posEdificio2);
    var modal360 = $('#modal-360');
    $(modal360).hide('fast');
    modal360.find('.content-modal').html('');
  ...
  $('.grupos').hover(function () {
    $(this).find(".textosmapa").show(300);
  }, function () {
    $(this).find(".textosmapa").hide(300);
  });
  ...
  $('.unidadfilter li').click(function (e) {
    ...
    for (x = 0; x < unidades.length; x++) {
      ...
      for (y = 0; y < tipo.length; y++) {
        if (tipo[y] == 'duplex' && unidades[x].duplex == 1) {
          unidades[x].activa = 1;
          unidades[x].tipo = 1;
        } else if (tipo[y] == 'patio' && unidades[x].patio == 1) {
          unidades[x].activa = 1;
          unidades[x].tipo = 1;
        } else if (tipo[y] == 'balcon' && unidades[x].balcon == 1) {
          unidades[x].activa = 1;
          unidades[x].tipo = 1;
        } else if (tipo[y] == 'balconterraza' && unidades[x].terraza == 1) {
          unidades[x].activa = 1;
          unidades[x].tipo = 1;
        } else if (tipo[y] == 'dpiscina' && unidades[x].pileta == 1 && unidades[x].piso != 9) {
          unidades[x].activa = 1;
          unidades[x].tipo = 1;
        } else if (tipo[y] == 'triplex' && unidades[x].triplex == 1) {
          unidades[x].activa = 1;
          unidades[x].tipo = 1;
        }
      }
    ...
    if (piso7 > 0) {
      $("#piso7").addClass("active");
    }
    if (piso8 > 0) {
      $("#piso8").addClass("active");
    }
    if (piso9 > 0) {
      $("#piso9").addClass("active");
    }
  ...
  $(document).on("click", ".pisos", function () {
    ...
    $.ajax({
      url: "pisos/" + ide, success: function (result) {
  ...
  $(document).on("click", ".unidades", function () {
    ...
    $.ajax({
      url: "unidad/" + ide, success: function (result) {
